...
class SessionManager {
  ...
  buildSshConfig(conn) {
    ...
    if (conn.authMethod === 'key') {
      // keyData: key content uploaded into the encrypted vault; keyPath: a
      // file on disk. Either works; uploaded content wins.
      if (conn.keyData) cfg.privateKey = Buffer.from(conn.keyData, 'utf8');
      else if (conn.keyPath) cfg.privateKey = fs.readFileSync(conn.keyPath);
      else throw new Error('No private key set for this connection');
